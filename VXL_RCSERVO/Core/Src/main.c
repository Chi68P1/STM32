...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void RCservo(float Angle) {
	__HAL_TIM_SET_COUNTER(&htim1,0);
	__HAL_TIM_SET_COUNTER(&htim2,0);
	while(__HAL_TIM_GET_COUNTER(&htim1) < 6000+(uint16_t)(200*Angle/9)) {
		// empty
	}
	HAL_GPIO_TogglePin(SIGNAL_GPIO_Port, SIGNAL_Pin);
	
	while(__HAL_TIM_GET_COUNTER(&htim2) < 40000) {
		// empty
	}
	HAL_GPIO_TogglePin(SIGNAL_GPIO_Port, SIGNAL_Pin);
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
		/*if (htim == &htim1){    //check if it is tim1 overflow
			HAL_GPIO_TogglePin(SIGNAL_GPIO_Port, SIGNAL_Pin);
			HAL_TIM_Base_Stop_IT(&htim1);
		}
		else if (htim == &htim2){    //check if it is tim1 overflow
			HAL_GPIO_TogglePin(SIGNAL_GPIO_Port, SIGNAL_Pin);
