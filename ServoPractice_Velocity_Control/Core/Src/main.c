...
#define PULSES_PER_ROUND		500			/* So xung tren 1 vong cua dong co */
...
/* Thong so ham PIController */
float 		Kp 							= 0.168017241;				/* Ti le */
float 		Ki 							= 4.310344828;				/* Tich phan */
float 		Kd 							= 0.0;								/* Vi phan */
float			Kb 							= 25.65418163;				/* Dung cho Anti-windup */
...
/* ====================================== Ham printf => gui du lieu qua serial (len C#)====================================== */
struct __FILE
{
  int handle;
...
/* ================================================= Ket thuc ham dieu khien van toc su dung bo PI  =================================================== */
...
void motorRot(uint8_t runMode, int32_t pwmSet)
{
	...
	{	
		...
		__HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_2, pwmSet);
	}
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
	HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_2);						// Enable PWM
	HAL_TIM_Base_Start_IT(&htim4);											// Enable interrupt Timer 4
	HAL_TIM_Base_Start_IT(&htim5);											// Enable interrupt Timer 5
...
{		
	...
	if(GPIO_Pin == GPIO_PIN_6)
	{
		...
		VelCnt++;
		if (CountValue>=4*PULSES_PER_ROUND) {
			CountValue = 0;
			PosCnt++;
		}
		else if	(CountValue<=-4*PULSES_PER_ROUND) {
			CountValue = 0;
			PosCnt--;
		}
	}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {	
	...
	if(htim->Instance==TIM4)																												/* Ngat timer 4: 5ms => tinh PID */
	{
		VelTemp		 	= VelCnt;																													/* Gan bien dem van toc vao bien tam */		
		VelCnt 			= 0;																															/* Reset bien dem van toc */	
		CurrSpeed 	= 60*VelTemp/(4*PULSES_PER_ROUND*0.005);																			/* RPM */				
		CurrVel 		= 2*3.1415*VelTemp/(4*PULSES_PER_ROUND*0.005);																/* rad/s */	

		if (run==true){					
			PWMSet = PIController(DesiredVel, CurrVel, Kp, Ki, Kb, UHiLim, ULoLim);			/* Tinh pwm bang bo dieu khien PI */
			dir = 1;																																		/* Chon chieu quay CW */
			motorRot(dir,PWMSet);																												/* Xet chieu va pwm cho dong co */
		}
		return;
	}
	
	if(htim->Instance==TIM5)																												/* Ngat timer 5: 10ms => gui gia tri len C# bang ham printf */
	{
		tick++;	
		if ((run==true)&&(tick==5)){																									/* 10ms x 5 */
			tick = 0;
			printf("V%d\r \n",CurrSpeed);
		}
	}
