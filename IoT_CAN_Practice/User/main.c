...
//Giúp cho mình tao lênh print dê ktra du lieu nhan ve
#ifdef __GNUC__
  ...
  while (1)
	{
      ...
      printf("CAN_DATA7 %x \r\n",CAN_DATA7); */
		...
		if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_1) == 0)  //nút User
		{ 
				button = 0X02;
		}
		if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0) == 0) //nút Wakeup
		{ 
				button = 0X01;
		}
		
		CanWriteData(0xF001, button);  //Thay thành ID of Tester i: 0xF001 cho bài ktra
		button =0X00;
...
void Delay(void)  //tao hàm delay 200ms
{
